...
def collect_data(handle,fw,sock,data_buf,constants,locks):
    ...
    while True:
        try:
            ...
            all_data = list(ret[0])
            print("Number of samples on ljm buffer: %d",ret[2])
            ljm_buff = ret[2]
            while (ljm_buff != 0):
                more_data = ljm.eStreamRead(handle)
                all_data += more_data[0]
                ljm_buff = more_data[2]
                print("Number of samples on ljm buffer: %d",ljm_buff)
                i += 1
            # start = datetime.now
